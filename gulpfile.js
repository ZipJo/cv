...
gulp.task('fileinclude', function () {
	return gulp
		.src(developmentFolder + '/index_dev.html')
...
gulp.task('dev', gulp.series(gulp.parallel('fileinclude'), gulp.parallel('watch', 'serve')));
...
gulp.task('compile-index', function () {
	return gulp
		.src(developmentFolder + '/index.html')
		.pipe(minifyInline())
...
gulp.task('minify-index', function () {
	return gulp
		.src(productionFolder + '/index.html')
		.pipe(
			htmlmin({
				removeComments: true,
...
gulp.task('move', function () {
	return gulp
		.src(developmentFolder + '/**', { dot: true })
		.pipe(filter.include(moveFiles))
		.pipe(gulp.dest(productionFolder));
});

//main build task
gulp.task('build', gulp.series('fileinclude', 'compile-index', 'minify-index', 'move'));
...
gulp.task('sftp', function () {
	return gulp.src(productionFolder + '/**', { dot: true }).pipe(sftp(sftpParams));
});
